...
from ds_factory import make_element
...
def create_nvurisrcbin_bin(name, uri, logger, file_loop=False):
    ...
    uri_src_bin.connect("pad-added", cb_newpad_nvurisrcbin, (nbin, logger))
...
def create_rtsp_output_bin(name, codec, bitrate, enc_type, platform_info, logger):
    ...
    Returns:
        (bin, udp_port): the Gst.Bin and the UDP port used by udpsink
    """
    ...
    # Caps: NVMM for HW encoder, raw for SW encoder
    ...
    # Ghost sink pad from nvvideoconvert
...
def create_source_bin(index, uri, logger, file_loop=False):
    ...
    if uri.startswith("/dev/video"):
        ...
        # Convert local file paths to file:// URI for nvurisrcbin
