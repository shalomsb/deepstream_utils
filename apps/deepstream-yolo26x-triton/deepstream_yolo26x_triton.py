...
def main():
    config = Config()
    logger = Logger("deepstream-yolo26x-triton")
    platform_info = PlatformInfo()
    Gst.init(None)

    pipeline = create_pipeline("deepstream-yolo26x-triton", logger)
    ...
    # Parse YOLO26 post-NMS tensors -> NvDsObjectMeta (before tracker)
