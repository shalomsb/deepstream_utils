...
Runs each of yolo26{x,l,m,s,n} on the same image using the EXACT 6-tile
geometry from apps/deepstream-yolo26x-sahi-triton/ (640×640 source tiles into
640×640 model input — NO effective zoom). Hand-rolled GREEDYNMM with IoS
merges across tiles.
"""
...
# Tiles are (y_start, x_start) in a 1280×720 frame; tile size is 640×640.
SAHI_640_TILES_YX = [
    (0,  0), (0,  512), (0,  640),
    (80, 0), (80, 512), (80, 640),
]
...
def main():
    ...
    print(f"SAHI geometry: {len(SAHI_640_TILES_YX)} tiles of 640×640 (no zoom)")
    ...
    for idx, name in enumerate(MODELS, start=1):
        ...
        # Warmup (one tile is enough to compile CUDA kernels)
        _ = model.predict(img[:640, :640], imgsz=640, conf=args.conf, verbose=False)

        t0 = time.perf_counter()
        dets = run_sahi_tiled(model, img, tiles, imgsz=640, conf=args.conf,
                              ios_thr=args.ios)
        ...
        cv2.imwrite(str(out_dir / f"{idx}_{name}_sahi640.jpg"), annot)
        del model

    if annotated:
        # 5 cells → 3 cols × 2 rows; pad one blank if needed.
        cols = 3
